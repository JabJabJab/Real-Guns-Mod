...
function ORGMLoadManager:ammoSearch(weapon) --searches player inventory for usable ammo, returns the first usable ammo on the list
	local testData = weapon:getModData()
...
function ORGMLoadManager:magSearch(weapon) --searches player inventory for usable mags, returns the first usable mag on the list
	local testData = weapon:getModData()
	local tablename = testData.magType
	local magTable = _G[tablename]
	if magTable ~= nil then
		for i,v in ipairs(magTable) do
			local testMag = v;
...
function ORGMLoadManager:mostLoadedClipSearch(weapon) --returns the most loaded clip more than 0
	local mostAmmo = 0;
	local clip = nil
	local testData = weapon:getModData()
	local tablename = testData.magType
	...
	if clipTable ~= nil then
		for i = 0, items:size()-1 do
			...
			for i,v in ipairs(clipTable) do
				local testClip = v;
...
function ORGMLoadManager:mostLoadedSearch(weapon) --returns the most loaded mag or any mag if they are all empty
	local mostAmmo = -1;
	local mag = nil
	local testData = weapon:getModData()
	local tablename = testData.magType
...
function ORGMLoadManager:isWeaponReloadable(weapon)
	...
	if weaponData.currentCapacity  < weaponData.maxCapacity then --if the gun is fully loaded return false
		local ammoAvailable = false
		local difficulty = self.getDifficulty()
		if weaponData.loadStyle == 'magfed' then
			if difficulty == 1 then
				if self.ammoSearch(weapon) ~=nil then
					return true
				end
			else
				if self.magSearch(weapon) ~= nil or weaponData.containsMag == 1 then
					return true
				end
			end
		elseif weaponData.speedLoader ~= nil and weaponData.speedLoader == 1 then
			if difficulty == 1 then
				...
				if self.mostLoadedClipSearch(weapon) ~= nil or self.ammoSearch(weapon) ~= nil then
					return true
				end
			end
		else
...
function ORGMLoadManager:startOpening() --starts the close action
	local player = getPlayer()
	local moodles = player:getMoodles();
	local panicLevel = moodles:getMoodleLevel(MoodleType.Panic);
	self.loadable = ORGMLoadUtil:getLoadableWeapon(self.loadWeapon, player);
...
function ORGMLoadManager:stopLoadSuccess() --either continues the loading script for loose rounds or, ends it and clears the queue
	...
	if(self.loadWeapon ~= nil and self:loadStarted() == true) then
		...
		elseif self.openClose == 1 and isOpen ~= nil and isOpen ~= 0 then
			self:startClosing();
		else
			self:stopLoad() --stops
		end
...
function ORGMLoadManager:checkLoadConditions() -- allows reloading/unloading using the keyboard
	...
	if not (reloadKey or reloadButton or unloadKey) or self.loadStarted() or self.rackingStarted() then return end
	self.loadWeapon = playerObj:getPrimaryHandItem(); --checks for a gun in the primary hand
	if self.reloadWeapon == nil then --if there is nothing in the main hand it stops the script
		return;
	end
	if (reloadKey or reloadButton) then --actions if the reload key is pressed
		if self:isWeaponReloadable(self.loadWeapon) then --checks if the weapon is reloadable
			...
			if self.loadWeapon:getModData().loadStyle == 'magfed' and difficulty ~= 1 then
				self.reloadAmmo = mostLoadedSearch(self.loadWeapon) --otherwise it looks for the most loaded magazine, or any magazine if all are empty
				self:startLoading();
			elseif self.loadWeapon:getModData().speedLoader ~= nil and self.loadWeapon:getModData().speedLoader ~= 1 and difficulty ~= 1 then
				self.reloadAmmo = mostLoadedClipSearch(self.loadWeapon) --returns the most loaded speedloader, will not return anything if they are empty
				self:startLoading();
			else
				self.reloadAmmo = ammoSearch(self.loadWeapon) --returns first bit of ammo available from the list
				self:startLoading();
			end
		else
...
function ORGMLoadManager:checkRackConditions()
	...
	self.reloadable = ORGMLoadUtil:getLoadableWeapon(self.reloadWeapon, playerObj); --gets weapon info
...
aaa.startLoadHook = function(pl) --the hook to allow key press loading
    if pl then
  	    return ORGMLoadManager[pl:getPlayerNum()+1]:checkLoadConditions();
    else return ORGMLoadManager[1]:checkLoadConditions(); end
end
